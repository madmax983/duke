...
/// Generates a static analysis report in JSON format for the given class file.
#[must_use]
pub fn generate_analysis_json(cf: &ClassFile) -> String {
    let mut out = String::new();
    let class_name = resolve_class_name(cf, cf.this_class);

    let _ = write!(&mut out, "{{\"class\":\"{class_name}\",\"methods\":[");
    ...
    for method in &cf.methods {
        ...
        for attr in &method.attributes {
            if let AttributeData::Code(code) = &attr.data {
                ...
                if let Ok(instructions) = decode(&code.code) {
                    ...
                    let _ = write!(
                        &mut out,
                        "{{\"name\":\"{full_name}\",\"complexity\":{complexity},\"code_size\":{code_size}}}"
                    );
                } else {
                    if !first {
                        let _ = write!(&mut out, ",");
                    ...
                    let _ = write!(
                        &mut out,
                        "{{\"name\":\"{full_name}\",\"error\":\"decode error\",\"code_size\":{code_size}}}"
                    );
                }
            }
        }
        ...
        if !found_code {
            ...
            let _ = write!(
                &mut out,
                "{{\"name\":\"{full_name}\",\"complexity\":null,\"code_size\":0}}"
            );
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_generate_analysis_json() {
        let cf = ClassFile {
            major_version: 61,
            minor_version: 0,
            constant_pool: vec![
