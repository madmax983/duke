...
pub mod sync;
pub use sync::{
    AtomicPayload, ConditionState, ConditionWaiter, CountDownLatchState, CountDownLatchWaiter,
    CyclicBarrierState, CyclicBarrierWaiter, ExecutorShared, ExecutorState, ExecutorTask,
    ExecutorTaskKind, ReadWriteLockState, ReadWriteLockViewKind, ReentrantLockState,
    SemaphoreState, SemaphoreWaiter,
};
mod mermaid;
    ...
    pub fn minor_collect_prepare(&mut self, roots: &[Slot]) {
        ...
        for obj in to_space.iter_mut().flatten() {
            ...
            if let Some(payload) = &obj.atomic_payload {
                AtomicPayload::patch_forwarded_reference(payload, forward_map);
            }
        }
        ...
        for (old_idx, obj) in old.iter_mut().enumerate() {
            ...
            let atomic_has_young =
                obj.atomic_payload
                    .as_ref()
                    .is_some_and(|payload: &AtomicPayload| {
