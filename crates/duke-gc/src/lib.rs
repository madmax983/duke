...
mod host_file_tests {
    ...
    fn test_host_file_io_errors() {
        ...
        assert!(
            matches!(res, Err(VmError::JavaException { ref class_name }) if class_name == "java/io/FileNotFoundException")
        );

        let res = gc.write_host_file_byte(999, i32::from(b'A'));
        assert!(
        ...
        let res = gc.write_host_file_byte(reader_id, i32::from(b'B'));
        assert!(
            matches!(res, Err(VmError::JavaException { ref class_name }) if class_name == "java/io/IOException")
        );
