...
fn test_host_file_operations() {
    ...
    let fd = heap.host.open_host_input_file(&dummy_path).unwrap();
    let b = heap.host.read_host_file_byte(fd).unwrap();
    assert_eq!(b, i32::from(b't'));
    heap.host.close_host_file(fd);

    // open_host_output_file
    let out_fd = heap.host.open_host_output_file(&dummy_path).unwrap();
    heap.host
        .write_host_file_byte(out_fd, i32::from(b'A'))
        .unwrap();
    heap.host.close_host_file(out_fd);
...
fn should_handle_open_read_write_close_cycle() {
    ...
    heap.host
        ...
        .unwrap();
    heap.host
        .write_host_file_byte(out_fd, i32::from(b'B'))
        .unwrap();
    heap.host.close_host_file(out_fd);

    let in_fd = heap.host.open_host_input_file(&dummy_path).unwrap();
    let b1 = heap.host.read_host_file_byte(in_fd).unwrap();
    let b2 = heap.host.read_host_file_byte(in_fd).unwrap();
    let b3 = heap.host.read_host_file_byte(in_fd).unwrap(); // EOF
    assert_eq!(b1, i32::from(b'A'));
    assert_eq!(b2, i32::from(b'B'));
    assert_eq!(b3, -1);

    heap.host.close_host_file(in_fd);
    std::fs::remove_file(dummy_path).unwrap();
}
...
fn open_host_input_file_not_found() {
    ...
    let result = heap.host.open_host_input_file(&dummy_path);
    assert!(
...
fn spawn_host_process_empty_command() {
    let mut heap = Heap::new();
    let result = heap.host.spawn_host_process(&[], None);
    assert!(
...
fn spawn_host_process_invalid_command() {
    ...
    let result = heap
        .host
        .spawn_host_process(&["/does/not/exist/executable".to_string()], None);
    assert!(
        matches!(result, Err(duke_runtime::VmError::JavaException { class_name }) if class_name == "java/io/IOException")
    );
}
...
fn read_write_invalid_host_file_handle() {
    let mut heap = Heap::new();
    assert!(
        matches!(heap.host.read_host_file_byte(999), Err(duke_runtime::VmError::JavaException { class_name }) if class_name == "java/io/IOException")
    );
    assert!(
        matches!(heap.host.write_host_file_byte(999, 10), Err(duke_runtime::VmError::JavaException { class_name }) if class_name == "java/io/IOException")
    );
}
...
fn process_wait_and_destroy_cycle() {
    ...
    let p_ids = heap.host.spawn_host_process(&cmd, None).unwrap();

    let wait_result = heap.host.wait_host_process(p_ids.process_id).unwrap();
    assert_eq!(wait_result, 0);

    // Waiting again should return the cached exit code
    let wait_result2 = heap.host.wait_host_process(p_ids.process_id).unwrap();
    assert_eq!(wait_result2, 0);

    // try_host_process_exit_value should also return the cached code
    let try_val = heap
        .host
        .try_host_process_exit_value(p_ids.process_id)
        .unwrap();
    assert_eq!(try_val, Some(0));

    // destroy on a finished process is a no-op
    heap.host.destroy_host_process(p_ids.process_id).unwrap();
}
...
fn socket_operations() {
    ...
    let res = heap.host.connect_socket("invalid.host.local:12345");
    assert!(res.is_err());

    // test operations on a server socket using a random port (0)
    let server_fd = heap.host.bind_server_socket("127.0.0.1:0").unwrap();
    let port = heap.host.server_socket_local_port(server_fd).unwrap();
    ...
    // closing sockets
    heap.host.close_host_file(client_fd.0);
    heap.host.close_host_file(client_fd.1);
    heap.host.close_host_file(server_fd);
}
