...
mod tests {
    ...
    fn test_extracted_attribute_parsers() {
        ...
        for attr in &cf.attributes {
            match &attr.data {
                crate::AttributeData::LineNumberTable(entries) => {
                    lnt_found = true;
                    assert_eq!(entries.len(), 1);
                    assert_eq!(entries[0].start_pc, 0);
                    assert_eq!(entries[0].line_number, 10);
                }
                crate::AttributeData::LocalVariableTable(entries) => {
                    found_lvt = true;
                    assert_eq!(entries.len(), 1);
                    assert_eq!(entries[0].start_pc, 0);
                    assert_eq!(entries[0].length, 10);
                }
                crate::AttributeData::Exceptions {
                ...
                crate::AttributeData::BootstrapMethods(entries) => {
                    found_bm = true;
                    assert_eq!(entries.len(), 1);
