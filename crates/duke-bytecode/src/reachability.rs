//! Graph reachability analysis for JVM bytecode.
//!
//! This module provides algorithms to traverse [`BasicBlock`] graphs, identifying
//! dead code and finding execution paths. It answers the "how can I get there?"
//! and "can I get there at all?" questions during bytecode analysis.
//!
//! The entry point for these analyses typically requires a list of basic blocks
//! generated by [`crate::build_basic_blocks`].
...
/// Gets the successor PCs for a given [`crate::BasicBlock`] based on its last instruction.
///
/// This determines where control flow can go immediately after the block finishes executing.
///
/// # Examples
///
/// ```
/// use duke_bytecode::{Instruction, build_basic_blocks};
/// use duke_bytecode::reachability::get_successors;
///
/// let instructions = vec![
///     (0, Instruction::Iconst0),
///     (1, Instruction::Ifeq(5)), // Branch to 1 + 5 = 6, or fallthrough to 4
...
///
/// // The first block ends with `ifeq`
...
///
/// A block is considered dead if there is no valid control flow path from the entry
/// point to that block. Unreachable blocks are returned as a list of their `start_pc` values.
///
/// # Examples
///
/// ```
...
///
/// let instructions = vec![
///     (0, Instruction::Goto(6)), // Unconditionally jump to 6
///     (3, Instruction::Iconst1), // This block is completely dead!
///     (4, Instruction::Ireturn),
...
///
/// This uses a breadth-first search to find the shortest sequence of [`crate::BasicBlock`]s
/// needed to reach the `target_pc` from the `start_pc`. Returns a sequence of `start_pc`
/// values representing the path, or `None` if the target is unreachable.
///
/// # Panics
///
...
///     (3, Instruction::Goto(7)), // Branch to 3 + 7 = 10
///     (6, Instruction::Ireturn), // Dead code
...
/// let blocks = build_basic_blocks(&instructions);
///
/// // Shortest path from block 0 to block 10 goes through block 3
