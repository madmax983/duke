...
/// let path = find_shortest_path(&blocks, 0, 10).expect("parent must exist if found is true");
...
pub fn find_shortest_path(
    ...
    if found {
        ...
        while curr != start_pc {
            ...
            curr = *parents
                .get(&curr)
                .expect("parent must exist if found is true");
        }
        path.push(start_pc);
        path.reverse();
        Some(path)
...
mod tests {
    ...
    fn test_find_shortest_path() {
        ...
        // Path to 8: 0 -> 8
        let path2 = find_shortest_path(&blocks, 0, 8).expect("parent must exist if found is true");
