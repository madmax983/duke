...
impl Instruction {
    /// Returns the stack effect `(pops, pushes)` of the instruction.
    #[must_use]
    ...
    /// Returns `true` if the instruction is a normal return (excluding `athrow` and `ret`).
    #[must_use]
    pub const fn is_normal_return(&self) -> bool {
        matches!(
            self,
            Self::Return
                | Self::Ireturn
