...
#[cfg(test)]
mod tests_lookupswitch {
    use super::*;

    #[test]
    fn test_decoder_lookupswitch_negative_npairs() {
        let code = [
            op::LOOKUPSWITCH,
            0,
            0,
...
#[cfg(test)]
mod tests_tableswitch {
    use super::*;

    #[test]
    fn test_decoder_tableswitch_negative_count() {
        let mut buf = vec![op::TABLESWITCH];
        buf.push(0);
        buf.push(0);
        buf.push(0); // padding
        buf.extend_from_slice(&0u32.to_be_bytes()); // default
        buf.extend_from_slice(&5u32.to_be_bytes()); // low
        buf.extend_from_slice(&2u32.to_be_bytes()); // high
