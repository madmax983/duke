...
#[cfg(test)]
mod tests_native_coverage {
    use super::*;
    use duke_classfile::parser;

    #[test]
    fn test_build_class_context_coverage() {
        // Mock a valid classfile from bytes
        let bytes = [
            0xCA, 0xFE, 0xBA, 0xBE, 0x00, 0x00, 0x00, 0x34, 0x00, 0x0A, 0x0A, 0x00, 0x03, 0x00, 0x07, 0x07,
            0x00, 0x08, 0x07, 0x00, 0x09, 0x01, 0x00, 0x06, 0x3C, 0x69, 0x6E, 0x69, 0x74, 0x3E, 0x01, 0x00,
            0x03, 0x28, 0x29, 0x56, 0x01, 0x00, 0x04, 0x43, 0x6F, 0x64, 0x65, 0x0C, 0x00, 0x04, 0x00, 0x05,
            0x01, 0x00, 0x04, 0x54, 0x65, 0x73, 0x74, 0x01, 0x00, 0x10, 0x6A, 0x61, 0x76, 0x61, 0x2F, 0x6C,
            0x61, 0x6E, 0x67, 0x2F, 0x4F, 0x62, 0x6A, 0x65, 0x63, 0x74, 0x00, 0x21, 0x00, 0x02, 0x00, 0x03,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x01, 0x00, 0x04, 0x00, 0x05, 0x00, 0x01, 0x00, 0x06,
            0x00, 0x00, 0x00, 0x11, 0x00, 0x01, 0x00, 0x01, 0x00, 0x00, 0x00, 0x05, 0x2A, 0xB7, 0x00, 0x01,
            0xB1, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        ];
        if let Ok(class_file) = parser::parse(&bytes) {
            let context = build_class_context(&class_file);
            assert_eq!(context.class_name, "Test");
        }
    }
}
