//! Standard library native method implementations.
//!
//! Contains all the `pub(crate) fn native_*` functions that provide JVM standard library functionality
//! (e.g., `java.lang.System.out.println`, `java.time.LocalDate.now`, etc).
...
use crate::{
    NativeControl, ReflectedMethodInfo, allocate_class_object, binary_name_to_internal_name,
    class_internal_name_from_ref, collect_public_reflected_methods, descriptor_parameter_part,
    internal_name_to_binary_name, lookup_public_reflected_field, lookup_public_reflected_method,
    parameter_descriptor_from_class_array,
};
use crate::{
    allocate_reference_array, allocate_reflection_member_object, class_key_from_ref,
    collect_public_reflected_fields, descriptor_class_slot_from_source, method_return_descriptor,
    parse_arg_count, parse_arg_descriptors, reflection_member_declaring_class_name_slot,
    reflection_member_declaring_class_slot,
};

use crate::{
    CallFrame, CallbackOps, NativeThreadAction, REFLECTION_MEMBER_ACCESSIBLE_FIELD,
    ReflectedClassInfo, box_reflection_return_value, build_reflection_invoke_args,
    descriptor_return_type, field_slot_idx, format_java_double, format_java_float,
    reflected_field_handle, reflected_method_handle, reflection_array_elements,
    reflection_member_name_slot, unbox_reflection_argument,
};
...
pub(crate) fn box_primitive_slot(slot: Slot, heap: &mut duke_gc::Heap) -> Slot {
    match slot {
        Slot::Int(v) => {
...
pub(crate) const BOOT_JAR_FILE_ARCHIVE_JAR_FILE_SLOT: usize = 1;
pub(crate) const BOOT_EXPLODED_ARCHIVE_ROOT_DIRECTORY_SLOT: usize = 0;
pub(crate) const BOOT_EXPLODED_ARCHIVE_MANIFEST_SLOT: usize = 2;
...
pub(crate) fn make_list_from_slots(
    elems: &[Slot],
    heap: &mut duke_gc::Heap,
    out: &mut dyn Write,
    control: &mut NativeControl,
...
pub(crate) fn make_set_from_slots(
    elems: &[Slot],
    heap: &mut duke_gc::Heap,
    out: &mut dyn Write,
    control: &mut NativeControl,
...
pub(crate) fn compare_treemap_keys(a: Slot, b: Slot, heap: &duke_gc::Heap) -> std::cmp::Ordering {
...
pub(crate) fn treemap_keys_equal(a: Slot, b: Slot, heap: &duke_gc::Heap) -> bool {
...
pub(crate) fn treeset_slot_sort_key(slot: Slot, heap: &duke_gc::Heap) -> Option<TreeSortKey> {
    match slot {
...
pub(crate) fn make_joining_collector(
    heap: &mut duke_gc::Heap,
    delimiter: &str,
    prefix: &str,
    suffix: &str,
...
pub(crate) fn make_int_stream(heap: &mut duke_gc::Heap, values: Vec<i32>) -> u64 {
...
pub(crate) fn make_optional_int(heap: &mut duke_gc::Heap, value: Option<i32>) -> u64 {
...
pub(crate) fn make_optional(heap: &mut duke_gc::Heap, value: Option<Slot>) -> u64 {
    let r = heap.allocate("java/util/Optional".to_string(), 1);
...
pub(crate) fn int_stream_elems(heap: &duke_gc::Heap, ref_: u64) -> Vec<i32> {
...
pub(crate) fn native_class_get_package_name(
    ...
    let package_name = if internal_name.starts_with('[') {
        ...
        internal_name
            .rsplit_once('/')
            .map_or_else(String::new, |(package, _): (&str, &str)| {
                package.replace('/', ".")
            })
    };
...
pub(crate) const fn decode_pct_hex(byte: u8) -> Option<u8> {
    match byte {
...
pub(crate) const BOOT_ARCHIVE_ENTRY_NAME_SLOT: usize = 0;
pub(crate) const BOOT_ARCHIVE_ENTRY_DIRECTORY_SLOT: usize = 1;
...
pub(crate) fn native_class_get_methods(
    ...
    let method_refs = collect_public_reflected_methods(ops, &class_key)?
        .into_iter()
        .map(|(declaring_class, method): (String, ReflectedMethodInfo)| {
            allocate_reflection_member_object(
                heap,
                "java/lang/reflect/Method",
                &declaring_class,
...
pub(crate) const COMPARE_TO_METHOD: &str = "compareTo";
pub(crate) const COMPARE_TO_OBJECT_DESC: &str = "(Ljava/lang/Object;)I";
pub(crate) const SORT_COMPARATOR_DESC: &str = "(Ljava/util/Comparator;)V";
...
#[inline]
pub(crate) const fn ordering_to_int(o: std::cmp::Ordering) -> i32 {
    match o {
...
pub(crate) fn stream_min_max_by_comparator(
    args: &[Slot],
    heap: &mut duke_gc::Heap,
    out: &mut dyn Write,
    ops: &mut dyn CallbackOps,
...
pub(crate) fn compare_slots_natural(
    a: Slot,
    b: Slot,
    heap: &mut duke_gc::Heap,
    out: &mut dyn Write,
...
pub(crate) fn apply_format_width(
    s: String,
    width: usize,
    left_align: bool,
    zero_pad: bool,
...
pub(crate) fn format_scientific(v: f64, prec: usize, upper: bool) -> String {
    if v == 0.0 {
...
pub(crate) mod tests {
    ...
    pub struct FixedCodeSourceOps {
        pub(crate) code_source: Option<String>,
    }
