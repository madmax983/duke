...
/// Bolt Optimization: Avoids O(N) heap allocations when converting Base64 byte output to String by seamlessly reusing the existing vector allocation in O(1) space via `String::from_utf8(vec).unwrap()`, instead of iterating and collecting.
