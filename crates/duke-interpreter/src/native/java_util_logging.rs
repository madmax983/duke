...
pub(crate) fn native_jul_logger_remove_handler(
    ...
    // ⚡ Bolt: Removed `heap.get(logger_ref)?.fields.clone()` to eliminate O(N) heap allocation.
    let retained = {
...
pub(crate) fn native_jul_log_manager_get_logger_names(
    ...
    let manager_ref = extract_ref_arg(args, 0)?;
    // ⚡ Bolt: Removed `heap.get(manager_ref)?.fields.clone()` to eliminate O(N) heap allocation.
    let names: Vec<Slot> = {
