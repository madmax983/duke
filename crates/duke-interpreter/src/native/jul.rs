...
fn jul_level_name_by_value(value: i32) -> Option<&'static str> {
    JUL_LEVELS
        .iter()
        .find_map(|(level_name, level_value)| {
            (*level_value == value).then_some(*level_name)
        })
}
...
fn jul_create_simple_formatter(heap: &mut duke_gc::Heap) -> Slot {
    Slot::Reference(
        Some(heap.allocate("java/util/logging/SimpleFormatter".to_string(), 0)),
    )
}
...
fn jul_manager_ref(heap: &mut duke_gc::Heap) -> Result<u64> {
    if let Some(manager_ref) = heap
        .find_string_backed_object("java/util/logging/LogManager", "singleton")
    {
...
fn jul_manager_find_logger_by_name(
    ...
    for idx in 0..count {
        ...
        if string_value_from_ref(heap, name_ref)? == name
            && let Some(Slot::Reference(Some(logger_ref))) = fields
                .get(logger_idx)
                .copied()
        {
...
fn jul_logger_name_string(
        ...
        .unwrap_or(Slot::Reference(None))
    {
        Slot::Reference(Some(name_ref)) => {
            Ok(Some(string_value_from_ref(heap, name_ref)?))
        }
...
fn jul_slot_to_text(heap: &duke_gc::Heap, slot: Slot) -> Result<String> {
    match slot {
        Slot::Reference(Some(obj_ref)) => {
            Ok(heap_object_to_string(heap.get(obj_ref)?, obj_ref))
        }
        Slot::Reference(None) => Ok("null".to_string()),
...
fn jul_format_record(heap: &duke_gc::Heap, record_ref: u64) -> Result<String> {
    let level_name = jul_level_name_from_slot(
        heap,
        jul_record_level_slot(heap, record_ref),
    )?;
...
fn jul_handler_allows_record(
    ...
    let record_value = jul_level_value_from_slot(
        heap,
        jul_record_level_slot(heap, record_ref),
    )?;
...
fn jul_publish_record_to_logger(
    ...
    for idx in 0..handler_count {
        if let Some(Slot::Reference(Some(handler_ref))) = fields
    ...
    let use_parent_handlers = matches!(
        fields.get(JUL_LOGGER_USE_PARENT_HANDLERS_FIELD), Some(Slot::Int(value)) if *
        value != 0
    );
    if use_parent_handlers
        && let Some(Slot::Reference(Some(parent_ref))) = fields
            .get(JUL_LOGGER_PARENT_FIELD)
            .copied()
    {
...
fn jul_log_message_slot(
    ...
    jul_log_text(heap, out, logger_ref, level_slot, &message, params, thrown_slot)
}
...
pub(crate) fn native_jul_logger_get_anonymous_logger(
    ...
    Ok(
        Some(
            jul_create_logger(heap, None, Slot::Reference(None), true, parent_slot, &[])?,
        ),
    )
}
...
pub(crate) fn native_jul_logger_is_loggable(
    ...
    Ok(Some(Slot::Int(i32::from(jul_logger_is_loggable(heap, logger_ref, level_slot)?))))
}
...
pub(crate) fn native_jul_logger_add_handler(
    ...
    let count = match heap.get(logger_ref)?.fields.get(JUL_LOGGER_HANDLER_COUNT_FIELD) {
        Some(Slot::Int(count)) => usize::try_from((*count).max(0)).unwrap_or(0),
        _ => 0,
    };
...
pub(crate) fn native_jul_logger_remove_handler(
    ...
    heap.get_mut(logger_ref)?.fields.truncate(JUL_LOGGER_HANDLERS_START);
    ...
    Ok(
        Some(
            jul_manager_find_logger_by_name(heap, manager_ref, &name)?
                .map_or(
                    Slot::Reference(None),
                    |logger_ref| { Slot::Reference(Some(logger_ref)) },
                ),
        ),
    )
}
...
pub(crate) fn native_jul_handler_init(
    ...
    let all_level = jul_level_slot_by_name(heap, "ALL")?;
    heap.write_field(handler_ref, JUL_HANDLER_LEVEL_FIELD, all_level)?;
    heap.write_field(handler_ref, JUL_HANDLER_FORMATTER_FIELD, Slot::Reference(None))?;
    Ok(None)
}
...
pub(crate) fn native_jul_log_record_get_level(
    ...
    Ok(Some(jul_log_record_get_field(args, heap, JUL_LOG_RECORD_LEVEL_FIELD)?))
}
...
pub(crate) fn native_jul_log_record_get_message(
    ...
    Ok(Some(jul_log_record_get_field(args, heap, JUL_LOG_RECORD_MESSAGE_FIELD)?))
}
...
pub(crate) fn native_jul_log_record_get_logger_name(
    ...
    Ok(Some(jul_log_record_get_field(args, heap, JUL_LOG_RECORD_LOGGER_NAME_FIELD)?))
}
...
pub(crate) fn native_jul_log_record_get_thrown(
    ...
    Ok(Some(jul_log_record_get_field(args, heap, JUL_LOG_RECORD_THROWN_FIELD)?))
}
...
pub(crate) fn native_jul_log_record_get_millis(
    ...
    Ok(
        Some(
            match heap.get(record_ref)?.fields.get(JUL_LOG_RECORD_MILLIS_FIELD) {
...
pub(crate) fn native_jul_log_record_get_parameters(
    ...
    Ok(Some(jul_log_record_get_field(args, heap, JUL_LOG_RECORD_PARAMETERS_FIELD)?))
}
