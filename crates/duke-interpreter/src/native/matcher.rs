fn matcher_pattern_input(
    heap: &duke_gc::Heap,
    m_ref: u64,
) -> Result<Option<(u64, u64)>> {
    let fields = heap.get(m_ref)?.fields.clone();
    let Some(Slot::Reference(Some(pat_ref))) = fields.get(MATCHER_PATTERN_FIELD).copied()
    else {
        return Ok(None);
    };
    let Some(Slot::Reference(Some(input_ref))) = fields.get(MATCHER_INPUT_FIELD).copied()
    else {
        return Ok(None);
    };
...
fn matcher_field_int(
    ...
    Ok(
        match heap.get(m_ref)?.fields.get(field).copied() {
...
fn matcher_group_bounds(
    ...
    let Some((pattern_str, flags, input)) = matcher_pattern_input_text(heap, m_ref)?
    else {
    ...
    match group {
        ...
        MatcherGroup::Name(name) => {
            if !re.capture_names().flatten().any(|candidate| candidate == name) {
                return Err(
                    regex_illegal_argument(format!("No group with name <{name}>")),
                );
            }
...
fn matcher_group_text(
    ...
    Ok(
        matcher_group_bounds(heap, m_ref, group)?
...
pub(crate) fn native_matcher_matches(
    ...
    let input = heap.get(input_ref)?.string_value.clone().unwrap_or_default();
    let re = compile_java_regex_with_flags(&pattern_str, flags)?;
    let matched = re.find(&input).filter(|m| m.start() == 0 && m.end() == input.len());
...
pub(crate) fn native_matcher_group_n(
    ...
    let n = match args.get(1).copied() {
        ...
        Some(Slot::Int(n)) => {
            return Err(regex_index_out_of_bounds(format!("No group {n}")));
        }
        _ => 0,
    };
    if n == 0 {
...
pub(crate) fn native_matcher_group_name(
    ...
    let name = heap.get(name_ref)?.string_value.clone().unwrap_or_default();
...
pub(crate) fn native_matcher_append_replacement_sb(
    ...
    let replacement = heap
        ...
        .unwrap_or_default();
    let (match_start, match_end) = last_match_bounds(heap, m_ref)?;
    let append_pos = usize::try_from(
            matcher_field_int(heap, m_ref, MATCHER_APPEND_POS_FIELD, 0)?,
        )
        .unwrap_or(0);
    let Some((pattern_str, flags, input)) = matcher_pattern_input_text(heap, m_ref)?
    else {
    ...
    heap.get_mut(m_ref)?.fields[MATCHER_APPEND_POS_FIELD] = Slot::Int(
        i32::try_from(match_end).unwrap_or(i32::MAX),
    );
    Ok(Some(Slot::Reference(Some(m_ref))))
}
