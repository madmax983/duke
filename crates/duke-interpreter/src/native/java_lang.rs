...
///
/// ⚡ Bolt Optimization: Uses `.chars()` iterators directly instead of collecting to
/// `Vec<char>`s to avoid O(N) intermediate heap allocations.
...
fn compare_ignore_case(s1: &str, s2: &str) -> i32 {
    ...
    loop {
        match (iter1.next(), iter2.next()) {
            ...
            (None, None) => return 0,
            (Some(_), None) => return 1 + iter1.count() as i32,
            (None, Some(_)) => return -(1 + iter2.count() as i32),
        }
    }
}
