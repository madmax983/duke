...
pub use natives::bootstrap_stdlib;
        ...
        match &instr {
            ...
            Instruction::Invokedynamic(cp_idx) => {
                ...
                if bsm_class == "java/lang/invoke/StringConcatFactory" {
                    ...
                    let result = crate::natives::execute_string_concat_recipe(
                        &recipe,
                        &dynamic_args,
                        &arg_types,
                        &constants,
...
fn resolve_thread_entry(
    ...
    let target_ref = match heap
        .get(thread_ref)?
        .fields
        .get(crate::natives::THREAD_TARGET_SLOT)
    {
...
fn spawn_java_thread(
...
) -> VmResult<()> {
    {
        ...
        let already_started = matches!(
            thread.fields.get(crate::natives::THREAD_ID_SLOT),
    ...
    let entry = {
        ...
        {
            let thread = shared_guard.heap.get_mut(thread_ref)?;
            thread.fields[crate::natives::THREAD_ID_SLOT] = Slot::Int(thread_id);
        }
        let CompletionVm {
            registry,
            heap,
    ...
    for i in 1..elems.len() {
        ...
        while j > 0 {
            ...
            let cmp = ops.invoke(
                ...
                output,
                &class_name,
                crate::natives::COMPARE_TO_METHOD,
                crate::natives::COMPARE_TO_OBJECT_DESC,
...
fn native_collections_sort(
    ...
    ops.invoke(
        ...
        &class_name,
        "sort",
        crate::natives::SORT_COMPARATOR_DESC,
...
fn native_double_compareto(
    ...
    Ok(Some(Slot::Int(crate::natives::ordering_to_int(
        a.total_cmp(&b),
    ))))
}
