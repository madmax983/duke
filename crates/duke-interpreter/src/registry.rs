...
/// Represents a single frame in a Java stack trace generated by the interpreter.
///
/// Used primarily for populating stack traces when constructing Java `Throwable`
/// instances or executing `Thread.dumpStack()`. It maps internal execution state
/// back to human-readable source locations.
...
/// Control context provided to native method implementations.
///
/// When a native method executes, it operates outside the standard bytecode evaluation loop.
/// This struct allows native code to issue out-of-band commands back to the interpreter engine,
/// such as throwing an exception (`stack_trace`) or requesting a thread state change (like starting
/// a new thread).
