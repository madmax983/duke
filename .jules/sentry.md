...
## 2024-08-01 - Testing Java Native Sockets in Duke
**Learning:** Native `java_net` socket lifecycle operations (like `native_server_socket_accept` and `native_socket_init`) can be robustly tested locally without test flakiness by binding to an OS-assigned ephemeral port (using `0` for the port argument), fetching the assigned port via `getLocalPort()`, and using `std::sync::mpsc::channel` paired with `std::thread::spawn` to manage concurrent client-server handshakes without relying on fragile `sleep` statements. Additionally, when writing tests inside included files (e.g., `include!("native/java_net.rs")`), the test module must be given a unique name like `mod java_net_tests` to avoid colliding with `lib.rs`'s own `mod tests;`.
**Action:** Next time testing network operations or sockets, use `0` for the bound port to prevent cross-test CI collisions, always isolate blocking calls like `accept` in background threads synchronized via channels, and ensure test modules in `include!` files are uniquely named.
