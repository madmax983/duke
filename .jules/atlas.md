...
**[Facade for duke-classfile]
**Tangle:** The `duke-classfile` crate currently exposes its internal modules (`access_flags`, `attributes`, `class`, `constant_pool`, `error`, `parser`) as `pub mod` or `pub mod types`. This allows downstream consumers (like `duke-bytecode`, `duke-interpreter` and `duke` bin) to bypass the intended public API and reach into internal modules, violating the Facade pattern and increasing coupling.
**Blueprint:** Changed the visibility of internal modules in `duke-classfile` to `pub(crate) mod`. Ensured that all necessary public types are re-exported at the crate root (`lib.rs`) and updated downstream consumers to use the re-exports (e.g., `use duke_classfile::ClassFile;` instead of `use duke_classfile::types::ClassFile;`). This enforces clear module boundaries and structural cohesion.
