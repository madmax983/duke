...
**Blueprint:** Encapsulated both modules. In `duke_classfile`, changed `pub mod signature` to `pub(crate) mod signature` at the crate root, eliminating the `signature` namespace from the public API entirely. In `duke_telemetry`, reduced `ser_helpers` visibility to `pub(crate)`.
