...
**[Encapsulate `duke_classfile` and `duke-telemetry` Facades]**
**Tangle:** The `duke_classfile` API exported an intermediate `signature` module (`pub mod signature`) merely to re-export its inner types, and `duke-telemetry` exposed internal serialization helpers via `pub mod ser_helpers`. This creates confusing, leaky abstractions and redundant paths.
**Blueprint:** Encapsulated both modules. In `duke_classfile`, reduced `signature` visibility to `pub(crate)`, keeping the direct `pub use` statements at the crate root. In `duke_telemetry`, reduced `ser_helpers` visibility to `pub(crate)`.
