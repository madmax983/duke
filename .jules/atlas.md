...
**[Encapsulate Internal Modules]**
**Tangle:** The `duke_classfile` API exported an internal sub-module (`pub mod signature`) unnecessarily, violating encapsulation boundaries.
**Blueprint:** Encapsulated the `signature` module to `pub(crate)` to hide implementation details and maintain a clean public API, as its types are already explicitly re-exported.
