...
**[Encapsulate `duke_classfile` Signature Module]**
...
**Blueprint:** Encapsulated the module. In `duke_classfile`, replaced `pub mod signature` with `pub(crate) mod signature`, keeping the existing direct `pub use` statements at the crate root to eliminate the `signature` namespace from the public API entirely.
