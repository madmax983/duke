...
**Refactor Instruction Matching**
**Tangle:** `crates/duke-bytecode/src/cfg.rs`, `basic_block.rs`, and `call_graph.rs` had massive inline `match instr` blocks to determine instruction behaviors like `is_conditional_branch`, `is_return`, and `method_invocation_target`. This created "The Blob" and "The Shotgun" smells, where adding a new instruction meant updating matching blocks across the whole codebase.
**Blueprint:** Abstracted the matching logic behind new and existing helper methods directly on the `Instruction` enum (`is_conditional_branch()`, `method_invocation_target()`, `unconditional_jump_target()`). The upstream consumers now call these methods, drastically reducing redundancy.
