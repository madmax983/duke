...
**Confusion:** Public documentation linked to private items (e.g., `[`mark_old`]` and `[`KEEP_SYNTHETIC`]`), causing `private_intra_doc_links` warnings under `-D warnings`.
**Clarification:** Replaced bracketed links to private items with inline backticks (e.g., `` `Self::mark_old` `` and `` `KEEP_SYNTHETIC` ``) and removed trailing reference definitions to resolve the warnings.
