...
## 2024-05-25 - [Private Intra-Doc Links Warnings]
**Confusion:** Public documentation linking to private items (e.g. `mark_old`, `KEEP_SYNTHETIC`) causes `rustdoc::private_intra_doc_links` warnings when generating docs, leading to failed builds when `-D warnings` is enforced.
**Clarification:** Changed intra-doc links for private functions and constants (`[`mark_old`]`, `[`KEEP_SYNTHETIC`]`) to standard plain backticks (`` `mark_old` ``, `` `KEEP_SYNTHETIC` ``) to resolve the rustdoc warnings.
