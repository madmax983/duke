...
**Confusion:** Public documentation generated a `rustdoc::private_intra_doc_links` warning (escalated to an error by `-D warnings`) because it linked to internal (private) methods like `mark_old` and `KEEP_SYNTHETIC`.
**Clarification:** Replaced the bracketed intra-doc links with backticked code literals (e.g. `` `mark_old` ``) when referencing private items in public documentation to avoid breaking the build while keeping the text readable.
