...
**Confusion:** The `cargo doc` generation failed with `-D warnings` because some intra-doc links in `crates/duke-gc` and `crates/duke-interpreter` were pointing to private items.
**Clarification:** Fixed the broken intra-doc links in `crates/duke-gc/src/lib.rs` and `crates/duke-interpreter/src/registry.rs` by changing the links to plain backticked strings, as the linked items were private.
