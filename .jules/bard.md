...
**Confusion:** References to private items in public doc strings using bracket syntax (like `[`mark_old`]`) trigger the `rustdoc::private_intra_doc_links` lint, which breaks CI builds when `-D warnings` is enforced.
**Clarification:** Replaced private intra-doc links (`[item]`) with plain code blocks (`` `item` ``) when the target item is intentionally kept private (e.g., `pub fn` referencing an internal private helper function).
