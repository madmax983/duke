...
**Extract God Function (decode_one)**
**Learning:** `decode_one` in `crates/duke-bytecode/src/decoder.rs` contained an inline match arm for `op::POP..=op::SWAP` that included all the stack manipulation logic.
**Action:** Extract inline match logic into dedicated helper functions (e.g. `decode_stack_op`) to maintain a flat structure and ensure `decode_one` delegates uniformly across opcode categories.
