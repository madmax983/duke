...
**[Flattening Loop Pyramids]**
**Learning:** `load_jar_methods` in `duke/src/jar_diff.rs` had deep nesting (`if let Ok(...) { if let Ok(...) { ... } }`) inside a `for` loop, which makes the core logic hard to follow.
**Action:** When extracting results in a loop, use `let Ok(...) = ... else { continue; };` guard clauses to immediately skip to the next iteration on failure, allowing the main loop body to remain flat and readable.
