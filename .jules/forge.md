...
**Learning:** Deeply nested `if let Ok` loops in JAR analysis cause a "Pyramid of Doom" and make logic harder to read.
**Action:** Use guard clauses (`let Ok(...) = ... else { continue; }`) early in loops to reduce rightward drift and improve readability.
