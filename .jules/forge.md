...
**Extract God Function matches**
**Learning:** `decode_one` in `crates/duke-bytecode/src/decoder.rs` contained an inline match block for stack operations (`op::POP..=op::SWAP`), which increased the function's size and complexity, breaking the flattening constraint.
**Action:** Extract grouped operations in massive match blocks into dedicated helper functions (like `decode_stack_op`) placed outside and above the original function (but before its outer attributes) to maintain a flat structure and improve readability.
