...
**Extract Optional String Argument from Native Handler Arguments**
**Learning:** Many native handlers in `crates/duke-interpreter/src/native.rs` duplicated the boilerplate `match args.get(X) { Some(Slot::Reference(Some(r))) => ... }` simply to extract an optional string argument.
**Action:** Extracted this boilerplate into `extract_optional_string_arg_value(args, index, heap)?` and used `unwrap_or_default()` and `let ... else` guard clauses to completely eliminate dozens of 'Pyramid of Doom' match blocks.
