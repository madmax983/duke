...
**Extract Stack Effect Matchers**\n**Learning:** `stack_effect` in `crates/duke-bytecode/src/verifier.rs` is a massive, 250-line inline `match` statement to determine the pop and push count for every instruction. This inflates file size and duplicates structural knowledge of the instructions.\n**Action:** Always extract static metadata like stack effects into public helper methods directly on the enum (e.g., `Instruction::stack_effect()`) to encapsulate instruction semantics and flatten calling modules.
