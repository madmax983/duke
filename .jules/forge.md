...
**Refactoring boolean blindness in `synthetic_field`**
**Learning:** Functions accepting multiple positional booleans, like `synthetic_field(name, desc, is_static)`, suffer from "boolean blindness", reducing readability at call sites (e.g., `synthetic_field("foo", "I", false)`).
**Action:** Extract positional booleans and related parameters into a configuration struct (`SyntheticFieldConfig`) equipped with a `Default` implementation. This requires explicit named arguments at call sites, making the intention immediately obvious.
