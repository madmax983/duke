...
**Learning:** Manual loops that just collect diffs into new vectors add unnecessary verbosity and mutable state.
**Action:** Replace manual `for` loops with iterator pipelines (`.difference().map().collect()`).
