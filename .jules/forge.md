...
**Flattening Nested Blocks inside Iterators**
**Learning:** Using `if let Ok(x) = y` inside an iterator often causes deep nesting ("Pyramid of Doom").
**Action:** Extract nested blocks using early `continue` or early `return` with guard clauses like `let Ok(x) = y else { continue; };`. This significantly flattens the code and improves readability without changing semantics.
