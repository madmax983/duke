...
## 2024-05-03 - Refactored decode_one God Function in decoder.rs
**Learning:** A massive 300+ line match block handling every JVM opcode creates a severe Pyramid of Doom and decreases readability. However, breaking it into smaller functions based on opcode ranges must be done carefully to ensure coverage is not dropped, and clippy lints like `unnecessary_wraps` may appear if the helper functions don't inherently require a `Result` wrapper but are written to return one out of habit.
**Action:** Use contiguous ranges (e.g., `op::NOP..=op::LDC2_W`) to route logic cleanly to helper functions. Use `#[allow(clippy::unnecessary_wraps)]` or refactor the return types if the helper function only returns `Ok(Instruction)` and never an `Err`.
