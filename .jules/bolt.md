...
## 2023-10-24 - Arrays.toString formatting
**Learning:** Returning a `Vec` clone from `heap.get(arr_ref)?.fields.clone()` just to format the elements as a string creates unnecessary heap allocations and memory copies. This is a common pitfall because the formatting logic itself only needs read access, but a subsequent mutable operation (like `heap.allocate_string(result)`) forces the borrow checker to complain if the immutable borrow isn't scoped correctly.
**Action:** When a method needs to iterate over immutable fields and then perform a mutable heap operation with the result, wrap the immutable iteration logic in a lexical block (`let result = { ... };`). This ensures the immutable borrow on `heap` is dropped before the mutable borrow begins, eliminating the need to `.clone()` the `fields` vector.
