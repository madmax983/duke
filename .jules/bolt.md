...
## String Iteration
**Learning:** When comparing strings character by character, allocating intermediate vectors like `s.chars().collect::<Vec<char>>()` adds unnecessary O(N) heap allocations.
**Action:** Iterate directly over `s.chars()` using a loop and `.next()` to achieve zero-allocation O(1) space complexity.
