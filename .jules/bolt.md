...
**Learning:** Using `into_iter().map(char::from).collect()` to convert a `Vec<u8>` containing known ASCII data into a `String` causes unnecessary O(N) heap allocations.
**Action:** Use `String::from_utf8(vec).unwrap()` instead, which seamlessly reuses the existing vector allocation in O(1) space.
