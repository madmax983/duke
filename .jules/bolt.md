...
## 2024-05-14 - Avoided intermediate allocation for char collection
**Learning:** The `encode_base64` method accumulated char objects from a buffer, resulting in unnecessary heap allocation via `map(char::from).collect()`.
**Action:** Used `String::from_utf8(out).unwrap()` since the accumulated Base64 bytes are guaranteed to be valid ASCII.
