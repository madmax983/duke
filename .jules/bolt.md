...
## 2025-02-12 - Zero-cost Base64 String conversion
**Learning:** `Vec::into_iter().map(char::from).collect()` over a known ASCII byte array unnecessarily copies bytes and allocates an entirely new String on the heap.
**Action:** Use `String::from_utf8(vec).expect("...")` (or `.unwrap()`) when returning `String`s from vectors populated by Base64 encoders to reuse the underlying `Vec` capacity.
