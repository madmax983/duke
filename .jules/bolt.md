...
**reachability::find_dead_blocks & find_shortest_path O(N) Allocation Optimization**
**Learning:** Initializing a `HashMap` just to perform lookups on a naturally sorted slice (e.g. `blocks: &[BasicBlock]`) incurs an unnecessary $O(N)$ heap allocation and insertion cost. By replacing it with `binary_search_by_key`, we eliminate the allocation and improve cache locality, resulting in $O(\log N)$ lookup time. Ensure `binary_search` loops use cached array indices inside nested BFS queues to prevent duplicated search operations when popping queue items. Also, correctly order compound conditional checks (e.g., hash `contains` before `binary_search`) to prevent slow paths.
**Action:** Always prefer `binary_search` on slice parameters when they are known to be sorted instead of mapping them into a `HashMap` inside local scopes, and pre-resolve indexes whenever they are placed in intermediate structures like queues to skip redundantly searching for them later. Ensure to not fight clippy on `collapsible_if` either by keeping flat loops.
