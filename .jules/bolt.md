...
**Optimize string allocations returning from `duke-bytecode::call_graph::extract_method_ref`**
**Learning:** `extract_method_ref` used to return `Option<(String, String, String)>` but it was allocating completely new `String` instances on the heap for every method call on `invoke*` instructions just to construct the dependency call graph. Since it just formats them to build the call graph shortly after, we can return string slices referencing the constant pool (`Option<(&str, &str, &str)>`) saving 3 heap allocations per `invoke*` instruction in a class file.
**Action:** When a function returns a String that was extracted from a structured object like `duke_classfile::ClassFile` that has a backing `Vec<u8>` or similar read-only memory layout, consider using a string slice `&str` instead of `String` since the lifetime of the returned string slice can naturally tie to the `ClassFile` parameter without allocating intermediate strings on the heap.
