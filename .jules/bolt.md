...
**[Eliminate `format!` overhead vs Pre-allocation]**
**Learning:** Replacing `writeln!(string)` with `write!(buf)` + `string.push_str(&buf)` inside loops is a de-optimization that adds copying without removing formatting overhead.
**Action:** Instead, pre-calculate capacity using `String::with_capacity()` for the target string upfront to eliminate heap reallocations without losing the convenience of formatting macros.
